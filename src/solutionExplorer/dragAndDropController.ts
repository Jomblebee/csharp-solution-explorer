...
import { moveProjectToFolder, moveSolutionFolderInto } from "./moveOps.js";
  ...
  implements vscode.TreeDragAndDropController<SolutionExplorerTreeItem>
{
  ...
  private async handleStructureDrop(
    ...
    if (solutionUri.fsPath.toLowerCase().endsWith(".slnx")) {
      vscode.window.showInformationMessage(
        "C# Solution Explorer: Re-arranging items within .slnx solutions is not yet supported.",
      );
      return;
    }
    ...
    for (const item of items) {
      ...
      if (!itemSolutionUri || itemSolutionUri.fsPath !== solutionUri.fsPath) {
        continue; // Cannot move items across different solutions.
      }

      if (item instanceof ProjectTreeItem) {
        if (!item.info.guid || (item.info.parentFolderGuid ?? null) === targetFolderGuid) {
          continue; // Missing GUID, or already in the target — nothing to do.
        }
        slnText = moveProjectToFolder(slnText, item.info.guid, targetFolderGuid);
        changed = true;
      } else {
        if (item.info.guid === targetFolderGuid) {
          continue; // Dropped onto itself.
        }
  ...
  private async handleFsDrop(
    ...
    for (const item of items) {
      ...
      const sourceParent = path.dirname(sourceUri.fsPath);

      if (sourceParent === targetDir.fsPath) {
